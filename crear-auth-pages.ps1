# Crear paginas de auth
Write-Host "=== Creando paginas de autenticacion ===" -ForegroundColor Cyan

New-Item -ItemType Directory -Force -Path "src\app\auth\confirm" | Out-Null
New-Item -ItemType Directory -Force -Path "src\app\auth\reset-password" | Out-Null

# ── Pagina de confirmacion/callback ─────────────────────────
$content = @'
...
'@
Set-Content -Path "src\app\auth\confirm\route.ts" -Value $content
Write-Host "[OK] Auth confirm route" -ForegroundColor Green
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-[#0f1f12] border border-[#1a3318] rounded-2xl p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[11px] font-bold text-[#5a8a52] uppercase tracking-widest mb-1.5">
                Nueva contrasena
              </label>
              <input
...
'@
Set-Content -Path "src\app\auth\reset-password\page.tsx" -Value $content
Write-Host "[OK] Reset password page" -ForegroundColor Green

Write-Host ""
Write-Host "=== Listo ===" -ForegroundColor Cyan
Write-Host "Ejecuta: npx vercel --prod --force" -ForegroundColor Yellow
