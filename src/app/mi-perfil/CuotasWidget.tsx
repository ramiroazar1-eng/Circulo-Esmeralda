      ...
      {periods.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {periods.map((p: any) => {
            ...
            return (
              <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: "10px", background: st.bg, border: 1px solid 33 }}>
                <div>
                  <p style={{ fontSize: "13px", fontWeight: 600, margin: 0 }}>{MONTHS[p.period_month - 1]} {p.period_year}</p>
                  <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", margin: 0 }}>
                    {p.paid_at ? Aprobado  :
                     p.comprobante_uploaded_at ? Comprobante enviado  :
                     $}
                  </p>
                </div>
