...
export default function EscanearPage() {
  ...
  async function handleRoomScan(value: string) {
    ...
    try {
      ...
      setTimeout(() => router.push(`/r/${token}`), 100)
    } catch (e) {
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 pt-4">
      ...
      <div className="bg-[#f5faf3] border border-[#ddecd8] rounded-xl p-4">
        ...
        <p className="text-xs text-[#6b8c65]">Para dispensar: escaneÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ el QR del paciente y luego el del lote.</p>
        <p className="text-xs text-[#6b8c65] mt-1">Para registrar en sala: escaneÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡ el QR pegado en la pared de la sala.</p>
      </div>
    </div>
  )
}