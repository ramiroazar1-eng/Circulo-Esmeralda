...
export default async function MembresiasPage() {
  ...
  return (
    <div className="spa
cat > src/app/membresias/page.tsx << 'ENDOFFILE'
import { createClient } from "@/lib/supabase/server"
import { redirect } from "next/navigation"
import { PageHeader, Card, Table, EmptyState, SectionHeader, StatCard } from "@/components/ui"
import { BackButton } from "@/components/ui/BackButton"
    ...
    <div className="space-y-5">
      ...
      <Card padding={false}>
        <div className="px-5 pt-5 pb-4">
          <SectionHeader title={`Estado de pagos — ${MONTHS[currentMonth - 1]} ${currentYear}`} />
        </div>
        {patientList.length === 0 ? (
          ...
          <Table>
            <thead>
              <tr><th>Paciente</th><th>DNI</th><th>Plan</th><th>Monto</th><th>Estado</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              {patientList.map((p: any) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td>
                      {isPaid ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-[#edf7e8] text-[#2d6a1f] border border-[#b8daa8]">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#4caf35]" />Pagado
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-[#fdf8ec] text-[#8a6010] border border-[#e8d48a]">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#e8a820]" />Pendiente
                        </span>
                      )}
                    </td>
                    <td>
      ...
      <Card padding={false}>
        ...
        {(!allPayments || allPayments.length === 0) ? (
          ...
          <Table>
            <thead>
              <tr><th>Fecha</th><th>Paciente</th><th>Plan</th><th>Periodo</th><th>Monto</th><th>Metodo</th><th>Recibo</th></tr>
            </thead>
            <tbody>
